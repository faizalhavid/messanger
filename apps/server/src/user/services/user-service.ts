...
import { UserModelMapper, UserProfileRequest, UserProfileSchema, UserProfile, QueryParamsData, PaginatedData, PaginationMeta } from '@messanger/types';
...
export class UserService {
  ...
  static async updateDeleteUserStatus(userId: string, status: boolean): Promise<UserProfile> {
    ...
    if (status === true && user.deletedAt && user.deletedAt > new Date(Date.now() - this.PENDING_TIME_ROLLBACK_DELETED_USER_STATUS)) {
    ...
    if (status === false && user.deletedAt && user.deletedAt > new Date(Date.now() - this.PENDING_TIME_ROLLBACK_DELETED_USER_STATUS)) {
