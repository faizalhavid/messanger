...
describe('Thread API', () => {
    ...
    it('should get all conversations in a Thread by ID', async () => {
    ...
    it('should get all conversations in a Thread by ID with pagination and query params', async () => {
        const thread = await ThreadTest.create({
            id: 'thread-3',
            type: 'GROUP',
            creatorId: usersTest[0].id,
        ...
        await ConversationTest.create({
            id: 'conversation-1',
            content: 'This is a paginated conversation',
            senderId: usersTest[0].id,
            threadId: thread.id,
        });
        await ConversationStatusTest.create({
        ...
        await ConversationTest.create({
            id: 'conversation-2',
            content: 'This is another paginated conversation',
            senderId: usersTest[1].id,
            threadId: thread.id,
        });
        ...
        const response = await fetch(`http://localhost:3000/api/threads/${thread.id}?page=1&limit=10`, {
            method: 'GET',
            headers: { Authorization: usersTest[0].token },
        });
        console.log('Response:', response);
        ...
        expect(body.success).toBe(true);
        expect(body.data).toBeDefined();
        expect(body.data.meta.totalItems).toBeGreaterThan(0);
        expect(body.data.meta.totalItems).toBeGreaterThan(0);
    });
    ...
    it('should return 404 for non-existent thread', async () => {
        ...
        //  Todo : Need error handling for non-existent thread
